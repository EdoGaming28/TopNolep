...
use TopNolep\EdoGaming\database\SQLite;
...
class Main extends PluginBase implements Listener
{
    ...
    public function onEnable(): void
    {
     ...
     $this->getServer()->getLogger()->Info("Top Nolep Enable By Edo Gaming");
        /*$pos = $this->config["positions"];
        $this->particles[] = new FloatingText($this, new Vector3($pos[0], $pos[1], $pos[2]));
        $this->getScheduler()->scheduleRepeatingTask(new UpdateTask($this), 100);*/
    }
            ...
            if (isset($args[0])) {
                switch ($args[0]) {
                    ...
                    case "settopnolep":
                        ...
                        $config->save();
                        $sender->sendMessage("§a> Activated!");
                        break;
                    default:
    ...
    public function getLeaderBoard():string{
        ...
        $pagelength = 10;
        $page = 1;
        $title = "§a§lTOP NOLEP\n§710 Pemain Paling Nolep Di Server";
        foreach ($data as $key => $val) {
            $i++;
            if ($i >= $pagelength * ($page - 1) && $i <= (($pagelength * ($page - 1)) + 10)) {
                $session = in_array($key, $this->getServer()->getOnlinePlayers()) ? self::$times[$key] : 0;
    ...
    public function getNolepTime($pn): String
    {
        //By Edo Gemink V:
        $pn = "$pn";
        $pn = strtolower($pn);
        ...
        $format = "";
        if($t < 60){
            $format = ($t < 0 ? '-' : '') . floor(abs($t) % 60) . "s";
        } elseif($t > 60 && $t < 3600){
            $format = ($t < 0 ? '-' : '') . floor(abs($t) / 60) . "m";
        } elseif($t > 3600){
            $format = ($t < 0 ? '-' : '') . floor(abs($t) / 3600) . "h";
        }
        return $format;
    }
